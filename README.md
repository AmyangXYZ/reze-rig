...
- **IK, for when the floor matters.** Off by default: the source's joint angles are reproduced exactly, which is what a dance, a gesture or a fight animation wants — there the pose *is* the content. Switch **IK** on and the legs run through the model's own 左足ＩＫ / 右足ＩＫ chains, the body rises until the lower foot clears the ground, and a foot standing on the floor is pinned there instead of drifting out from under the character. Reach for it when contact carries the motion — walks and runs, crouches, anything where the feet must not sink or skate — or when the target model is built unlike the source. Contact is read off whichever end of the foot is lower, so a character up on her toes is not mistaken for one in mid-air.
- **In Place** removes horizontal travel while keeping the vertical, so jumps and crouches survive.
- **Skeleton** draws the rig the conversion actually produced over the model, which is the quickest way to tell a bad mapping from a bad pose.
